...
export class AdminController {
  ...
  // =========================
  // 危险操作：创建用户（仅 SUPER_ADMIN）
  // =========================

  @Post('users')
  @Roles('SUPER_ADMIN')
  @ApiOperation({ summary: '（SUPER_ADMIN）创建用户（自动生成强密码兜底 + Wallet 初始化 + 审计）' })
  @ApiBody({
    schema: {
      type: 'object',
      required: ['email', 'nickname', 'reason'],
      properties: {
        email: { type: 'string', format: 'email', example: 'newuser@example.com' },
        nickname: { type: 'string', example: '新用户' },
        password: { type: 'string', example: '可选，不填自动生成', description: '不填则自动生成 12 位强密码' },
        role: { type: 'string', enum: ['USER', 'ADMIN'], default: 'USER' },
        reason: { type: 'string', example: '新成员加入，需注册账号' },
      },
    },
  })
  async createUser(
    @Body() body: { email: string; nickname: string; password?: string; role?: string; reason: string },
    @Req() req: any,
  ) {
    const adminId = Number(req?.user?.id);
    ...
    // ── 校验 ──
    if (!body?.reason || !body.reason.trim()) {
      throw new BadRequestException('创建原因不能为空');
    }
    ...
    if (role === 'SUPER_ADMIN') {
      throw new BadRequestException('不能直接创建超级管理员');
    }
    if (!['USER', 'ADMIN'].includes(role)) {
      throw new BadRequestException('角色仅支持 USER 或 ADMIN');
    }
    ...
    // 密码策略
    const userProvidedPassword = body.password && body.password.trim() ? body.password.trim() : null;
    if (userProvidedPassword && userProvidedPassword.length < 6) {
      throw new BadRequestException('密码长度至少 6 位');
    }
    const rawPassword = userProvidedPassword || this.generateSecurePassword();
    ...
    // ── 初始化 Wallet ──
    try {
      await this.wallet.createWallet(newUser.id);
    } catch (walletError) {
      // 钱包初始化失败不阻塞用户创建
    ...
    await this.audit.log({
      ...
      detail: {
        ...
        passwordProvided: !!userProvidedPassword,
      },
    });
    ...
    return {
      ...
      role: newUser.role,
      temporaryPassword: userProvidedPassword ? null : rawPassword,
      passwordAutoGenerated: !userProvidedPassword,
      message: userProvidedPassword
        ? '用户创建成功'
        : '用户创建成功，请妥善记录下方临时密码（仅显示一次）',
    };
  }
